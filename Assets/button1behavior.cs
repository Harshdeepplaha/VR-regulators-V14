...
public class button1behavior : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        /* if (tatrumchildbehavior.childIsTalking || audioSource.isPlaying)
         {                                               // if the player've just clicked this button and the audio is play -> disable button so they can't click it again
             //yourButton.interactable = false;
             //tmpText.text = "";
         }
         else if (!audioSource.isPlaying){ // let the parent finish talking first, otherwise their audio is cut off during tantrum level change
             UpdateTextAndAudioClip(tantrumLevel);
             //yourButton.interactable = true;
         }*/
        if (atStart && introTimer < introLength || audioSource.isPlaying || childState.tantrumLevel == 0 || tatrumchildbehavior.gameLost)  // if we at start the child is saying the greeting -> disable the button
        {                                                                        // if the player've just clicked this button and the audio is play -> disable button so they can't click it again
            yourButton.interactable = false;
            //tmpText.text = "";
        }
